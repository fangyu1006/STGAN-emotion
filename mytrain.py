...
# settings
dataroot_default = '/home/iim/Data/deepglint_112x112'
...
# model
att_default = ['Mouth_Slightly_Open', 'Smiling']
parser.add_argument('--atts', default=att_default, choices=mydata.Deepglint.att_dict.keys(), nargs='+',
...
tr_data = mydata.Deepglint(dataroot, atts, img_size, batch_size, part='train', sess=sess, crop=crop_)
val_data = mydata.Deepglint(dataroot, atts, img_size, n_sample, part='val', shuffle=False, sess=sess, crop=crop_)

# models
Genc = partial(mymodels.Genc, dim=enc_dim, n_layers=enc_layers, multi_inputs=multi_inputs)
Gdec = partial(mymodels.Gdec, dim=dec_dim, n_layers=dec_layers, shortcut_layers=shortcut_layers,
               inject_layers=inject_layers, one_more_conv=one_more_conv)
Gstu = partial(mymodels.Gstu, dim=stu_dim, n_layers=stu_layers, inject_layers=stu_inject_layers,
               kernel_size=stu_kernel_size, norm=stu_norm, pass_state=stu_state)
D = partial(mymodels.D, n_att=n_att, dim=dis_dim, fc_dim=dis_fc_dim, n_layers=dis_layers)
...
if mode == 'wgan':  # wgan-gp
    ...
    d_loss_gan = -wd
    gp = mymodels.gradient_penalty(D, xa, xb_)
    ...
    gp = mymodels.gradient_penalty(D, xa)
...
try:
    ...
    for i in range(len(atts)):
        ...
        tmp = mydata.Deepglint.check_attribute_conflict(tmp, atts[i], atts)
